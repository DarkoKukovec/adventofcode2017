function getDistance(directions) {
  const map = directions.reduce((map, dir, index) => {
    map[dir]++;
    return map;
  }, {ne: 0, nw: 0, se: 0, sw: 0, s: 0, n: 0});
...
module.exports = (input) => {
  ...
  return directions.reduce((max, _, index, arr) => Math.max(max, getDistance(arr.slice(0, index))), 0);
}
