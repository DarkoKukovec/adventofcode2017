...
module.exports = (input) => {
  ...
  const maxLength = bridges.reduce((maxLength, item) => Math.max(maxLength, item.line.length), 0);
  const maxLengthBridges = bridges.filter((bridge) => bridge.line.length === maxLength);

  return maxLengthBridges
